...
uint_fast8_t count_total_nb_links()
{
    ...
    std::array<uint_fast8_t, 32> nb_modules_links;

    auto modules_links = application_.vie_processor_.orchestrator_.get_modules_links(nb_modules, nb_modules_links);

    uint_fast16_t nb_links = 0;
    //For each module.
    for (int i = 0; i < nb_modules; i++)
    {
		//For each link in each module.
        nb_links += nb_modules_links[i];
    }
        ...
        WHEN("We load the project") {
            ...
            THEN("The project headers are loaded from disk.") {
                ...
                AND_THEN("The modules previously added are also loaded.") {
                    ...
                    auto link = application_.vie_processor_.orchestrator_.modules_links_[0][0];

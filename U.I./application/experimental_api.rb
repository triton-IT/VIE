...
Essentials.new_default_params({ list: { colors: %i[red blue], sort: :alphabetically, height: 18, width: :auto,
                                        margin: 0, spacing: 3, view: :data, listing: {}, items:
                                          { color: :orange, size: 36, align: :left, margin: 3 } } })
new({ particle: :list })
...
new({ post: :list }) do |params|
  ...
  list_listing.each_with_index do |item, index|
    support = box({ top: (line_height + line_spacing) * index, height: line_height, width: line_width,
                    left: line_margin, right: line_margin })
    support.text({ data: item[:name], visual: { size: 12 } })
    code = item[:code] ||= list_code
    support.touch(true) do
      send(code, item) if code
    end
  end
end

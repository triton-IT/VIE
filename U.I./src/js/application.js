...
Opal.queue(function(Opal) {/* Generated by Opal 1.7.2 */
  var $def = Opal.def, $hash2 = Opal.hash2, $send = Opal.send, $truthy = Opal.truthy, $eqeq = Opal.eqeq, $rb_plus = Opal.rb_plus, $rb_times = Opal.rb_times, $to_ary = Opal.to_ary, $rb_divide = Opal.rb_divide, self = Opal.top, $nesting = [], $$ = Opal.$r($nesting), nil = Opal.nil, default_project_name = nil, logo_color = nil;

  Opal.add_stubs('puts,to_json,parse,require,element,box,[],vie_styles,grab,text,merge,matrix,detached,width,right,each_with_index,value,children,touch,cell,data,[]=,include?,id,each,attached,<<,==,clear_zone,vivify,wait,create_matrix,set_title,+,*,color,send,fill_toolzone,delete,children=,top,remove_active,webview_sender,alert,list,each_value,tool_list,svg_fetch,drag,/,height');
  
  
  $def(self, '$webview_receiver', function $$webview_receiver(msg) {
    var self = this;

    return self.$puts($$('JSON').$parse(msg).$to_json())
  });
  
  $def(self, '$webview_sender', function $$webview_sender(params) {
    var json_msg = nil;
...
;
  ...
  $def(self, '$action_load', function $$action_load() {
    ...
    return self.$webview_sender($hash2(["action"], {"action": "get_projects"}));
  });

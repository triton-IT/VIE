...
namespace live::tritone::vie::vst {
	...
	private:
		core::Event to_event(Steinberg::Vst::Event& vst_event) const;

		core::NoteEvent to_note_event(Steinberg::Vst::NoteOnEvent& vst_note_on_event) const;

		core::NoteEvent to_note_event(Steinberg::Vst::NoteOffEvent& vst_note_off_event) const;

		core::DataEvent to_data_event(Steinberg::Vst::DataEvent& vst_data_event) const;

		void to_audio_bus_buffer(core::AudioBusBuffers* source, Steinberg::Vst::AudioBusBuffers& vst_audio_bus_buffers, int sample_size) const;
		...
		*/
		void process_input_data(core::AudioBusBuffers* source_buffers, Steinberg::Vst::AudioBusBuffers* buffers, int32_t nb_buffers, int32_t sample_size) const;

		core::IProcessor* vie_processor_;

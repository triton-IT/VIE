										...
										[this](ICoreWebView2* sender, ICoreWebView2WebMessageReceivedEventArgs* args) {
											if (handler_ != nullptr) {
                                                ...
                                                if(action_name.compare("set_module_parameter_value") == 0) {
													...
													core::IProcessor* processor = core::IProcessor::getInstance();
													core::IModule& processor_module = processor->getModule(module_id);
												    uint16_t parameter_module_id = processor_module.getId();
												    uint16_t parameter_slot_id = processor_module.getSlotId(parameter_id);

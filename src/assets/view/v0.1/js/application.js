    ...
    $def(self, '$receive_from_controller', function $$receive_from_controller() {
        if (window.webkit) {
            ...
            window.chrome.webview.addEventListener('message', arg => {
                console.log("action received : " + arg.data.action + "\nresponse: " + arg.data.response);
            });
        }
    });
    self.$receive_from_controller();

    $def(self, '$send_to_controller', function $$send_to_controller(json) {
        var self = this;
...
Opal.queue(function (Opal) {/* Generated by Opal 1.7.2 */
    ...
    $def(self, '$action_folder', function $$action_folder() {
        ...
        self.$send_to_controller({ "action": "get_projects" });
        self.$send_to_controller({ "action": "new_project" });
        self.$send_to_controller({ "action": "save_project" }, "body": { "path": "c:/tmp/project0.json" });
        self.$send_to_controller({ "action": "save_project_as" }, "body": { "path": "c:/tmp/project0.json" });
        self.$send_to_controller({ "action": "load_project", "body": {"path": "c:/tmp/project0.json"} });
        self.$send_to_controller({ "action": "get_modules" });
        self.$send_to_controller({ "action": "add_module", "body": { "type": "midi-in", "position": { "x": 0, "y": 0, "z": 0 } } });
        self.$send_to_controller({ "action": "set_module_name" });
        self.$send_to_controller({ "action": "delete_module" });
        self.$send_to_controller({ "action": "set_module_parameter_value" });
        self.$send_to_controller({ "action": "link_modules" });
        self.$send_to_controller({ "action": "undo" });
        return self.$send_to_controller({ "action": "redo" });
    });

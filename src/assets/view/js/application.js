Opal.modules["application/data_for_test"] = function (Opal) {/* Generated by Opal 1.7.2 */
    ...
    Opal.add_stubs('receive_from_controller,puts,to_json,parse');
    ...
    $def(self, '$receive_from_controller', function $$receive_from_controller() {
        if (window.webkit) {
            // write code here:
        } else {
            window.chrome.webview.addEventListener('message', arg => {
                console.log("data received from the controller : " + arg.data.action);
            });
        }
    });
    self.$receive_from_controller();

    $def(self, '$send_to_controller', function $$send_to_controller(msg) {
        var self = this, json_msg = nil;


        self.$puts(msg);
        json_msg = msg.$to_json();
        let json = JSON.parse(json_msg);
...
Opal.queue(function (Opal) {/* Generated by Opal 1.7.2 */
    ...
    Opal.add_stubs('require,element,box,[],vie_styles,grab,svg_fetch,text,merge,matrix,detached,width,right,each_with_index,value,children,touch,cell,puts,data,[]=,include?,id,each,attached,<<,==,clear_zone,vivify,wait,create_matrix,set_title,+,*,color,send,fill_toolzone,delete,children=,top,remove_active,send_to_controller,alert,list,each_value,tool_list,drag,/,height');
    ...
    $def(self, '$create_matrix', function $$create_matrix(id) {
        ...
        return $send(modules.$children().$value(), 'each_with_index', [], function $$1(_module_found, index) {
            var self = $$1.$$s == null ? this : $$1.$$s;


            if (_module_found == null) _module_found = nil;
            if (index == null) index = nil;
            $send(modules.$cell(index), 'touch', ["down"], function $$2() {
                var $a, self = $$2.$$s == null ? this : $$2.$$s;


                self.$puts("down context is: " + (self.$grab("context").$data().$value()));
                return ($a = [0, index], $send(self.$grab("connection").$data(), '[]=', $a), $a[$a.length - 1]);
            }, { $$s: self });
            $send(modules.$cell(index), 'touch', ["long"], function $$3() {
                ...
                if ($truthy(self.$grab("selected").$data().$value()['$include?'](self.$id().$value()))) {
                    ...
                    $send(self.$grab("selected").$data().$value(), 'each', [], function $$4(cell_id) {
                        var self = $$4.$$s == null ? this : $$4.$$s, selected_cell = nil;


                        if (cell_id == null) cell_id = nil;
                        selected_cell = self.$grab(cell_id);
                        selected_cell.$attached(["cell_color"]);
                        return selected_cell.$detached("active_color");
                    }, { $$s: self });
            ...
            return $send(modules.$cell(index), 'touch', ["up"], function $$5() {
                var self = $$5.$$s == null ? this : $$5.$$s, source_cell = nil, target_cell = nil, new_connection = nil;
    ...
    $def(self, '$fill_toolzone', function $$fill_toolzone(ids) {
        ...
        return $send(ids, 'each_with_index', [], function $$7(id_found, index) {
            var self = $$7.$$s == null ? this : $$7.$$s, support = nil;
            ...
            support.$color("red");
            return $send(support, 'touch', [true], function $$8() {
                var self = $$8.$$s == null ? this : $$8.$$s;

                return self.$send("action_" + (id_found))
            }, { $$s: self });
        }, { $$s: self });
    });
    self.$fill_toolzone(["folder", "clear", "settings", "edition", "select", "group", "copy", "paste", "undo"]);

    $def(self, '$action_clear', function $$action_clear() {
        var self = this;

        return $send(self.$grab("selected").$data(), 'each', [], function $$9(selected_slot) {
            var self = $$9.$$s == null ? this : $$9.$$s;


            if (selected_slot == null) selected_slot = nil;
            return $send(self.$grab(selected_slot).$children(), 'each', [], function $$10(child_found) {
    ...
    $def(self, '$clear_zone', function $$clear_zone(zone) {
        ...
        $send(zone.$children(), 'each', [], function $$11(child_found) {
    ...
    $def(self, '$remove_active', function $$remove_active(items) {
        ...
        return $send(items, 'each', [], function $$12(id_found, _properties) {
    ...
    $def(self, '$list', function $$list(parent, style, items) {
        ...
        margin = self.$vie_styles()['$[]']("margin");
        return $send(items, 'each_with_index', [], function $$13($mlhs_tmp1, index) {
            var $a, $b, self = $$13.$$s == null ? this : $$13.$$s, id_found = nil, properties = nil, label = nil, item = nil;
            ...
            return $send(item, 'touch', [true], function $$14() {
                ...
                return self.$send(properties['$[]']("action"), properties.$merge($hash2(["id"], { "id": id_found })));
            }, { $$s: self });
    ...
    $def(self, '$action_folder', function $$action_folder() {
        ...
        self.$send_to_controller($hash2(["action"], { "action": "get_projects" }));
        self.$send_to_controller($hash2(["action"], { "action": "new_project" }));
        self.$send_to_controller($hash2(["action"], { "action": "save_project" }));
        self.$send_to_controller($hash2(["action"], { "action": "save_project_as" }));
        self.$send_to_controller($hash2(["action"], { "action": "load_project" }));
        self.$send_to_controller($hash2(["action"], { "action": "get_modules" }));
        self.$send_to_controller($hash2(["action"], { "action": "add_module" }));
        self.$send_to_controller($hash2(["action"], { "action": "set_module_name" }));
        self.$send_to_controller($hash2(["action"], { "action": "delete_module" }));
        self.$send_to_controller($hash2(["action"], { "action": "set_module_parameter_value" }));
        self.$send_to_controller($hash2(["action"], { "action": "link_modules" }));
        self.$send_to_controller($hash2(["action"], { "action": "undo" }));
        return self.$send_to_controller($hash2(["action"], { "action": "redo" }));
    });
    ...
    $def(self, '$action_edition', function $$action_edition() {
        ...
        icon_spacing = $rb_plus(self.$vie_styles()['$[]']("support_style")['$[]']("height"), $rb_times(margin, 2));
        index = 0;
        return $send(self.$tool_list(), 'each_value', [], function $$15(data) {
            var self = $$15.$$s == null ? this : $$15.$$s, icon_found = nil, action_found = nil, id_found = nil, support = nil;
            ...
            $send(support, 'touch', [true], function $$16() {
                var self = $$16.$$s == null ? this : $$16.$$s;

                return self.$send(action_found, id_found)
            }, { $$s: self });
            support.$drag(true);
    ...
    $def(self, '$insert_module', function $$insert_module(module_id) {
        ...
        return $send(self.$grab("selected").$data(), 'each', [], function $$17(module_id_found) {
            var self = $$17.$$s == null ? this : $$17.$$s, module_found = nil, tool_found = nil, tool_color = nil;


            if (module_id_found == null) module_id_found = nil;
            module_found = self.$grab(module_id_found);
            $send(module_found.$children(), 'each', [], function $$18(child_found) {
            ...
            tool_color = "orange";
            module_found.$box($hash2(["id", "width", "height", "center", "attached"], { "id": "" + (module_found.$id().$value()) + "_svg_support", "width": $rb_divide(module_found.$width().$value(), 2), "height": $rb_divide(module_found.$height().$value(), 2), "center": true, "attached": "invisible_color" }));
            return self.$svg_fetch(tool_found, tool_color, "" + (module_found.$id().$value()) + "_svg_support");
        }, { $$s: self })
    });
